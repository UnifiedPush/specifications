...
* VAPID public key: This is a public key on the P-256 curve encoded in the uncompressed form [SEC 1] (section 2.3.3, replicated from X9.62), and base64url encoded [RFC7515]. This is a 87 bytes long string. The public key is provided by the end user application, and send during the registration intent ([org.unifiedpush.android.distributor.REGISTER]). It is used by the application server to authenticate itself using the Voluntary Application Server Identification [RFC8292] protocol. Some push servers require the application server to authenticate with VAPID, then, if a valid VAPID key is not provided during the registration, the associated distributor will respond with a ([org.unifiedpush.android.connector.REGISTRATION_FAILED]) error.
...
2. C request one or more registration, the token is different for each registration.
    1. C->D: [org.unifiedpush.android.distributor.REGISTER] SDK<34: (extra: token, pi) SDK>=34: (extra: token, flag: FLAG_SHARE_IDENTITY), optionally: (extra: vapid, message)
...
It SHOULD be sent the following extra:

* vapid (String, 87 bytes):  a VAPID public key as defined in the [Resources]. Some push servers require the application server to authenticate push requests using VAPID. For those push servers, if the VAPID key is not provided by the connector, the distributor MUST respond with [org.unifiedpush.android.connector.REGISTRATION_FAILED] and set the reason to 'VAPID_REQUIRED'.
