...
If the intent's data and the callingPackage exist, the result is set to RESULT_OK (-1) with an intent containing the auth token, as defined in the [Resources], in the extra `auth_token` and the distributor packageName in the extra `application`.
...
This intent MUST contain 1 extra:
