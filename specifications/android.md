...
* Temporary link Token: This is a randomly generated token, generated by the connector for a link request. The end user application use it to link to a distributor, to receive the auth token. The distributor don't have to save this token. The connector can remove this token once the auth token is received. This token must contain sufficient entropy so it cannot be guessed. To generate this token, UUIDv4 ([RFC9562]) is suggested. It is unique on distributor and connector side. This is a string of maximum 100 bytes.
* Auth Token: This is a randomly generated token, generated by the distributor during a link request. The end user application use it to request new registrations. Its aim is to ensure that registration requests originate from the legitimate application, to prevent an illegitimate application from usurping the end user application ID in order to spam the distributor with registration requests. This token must contain sufficient entropy so it cannot be guessed. To generate this token, UUIDv4 ([RFC9562]) is suggested. It is unique on distributor and connector side. This is a string of maximum 100 bytes.
...
1. C and D link each other
    1. C->D: distributor.LINK (extra: application, token)
    2. C<-D: connector.LINKED (extra: auth, token)
...
* token (String): this is the temporary link token as defined in the [Resources].
...
The distributor generate a single auth token, as defined in the [Resources], for an application. If it receives a new LINK request for the same application, the distributor MUST send the same token.
...
* auth (String): this is the auth token as defined in the [Resources]. It has been received with the previous [org.unifiedpush.android.connector.LINKED] intent.
...
* auth (String): this is the auth token as defined in the [Resources]. This is a uniq token generated by the distributor, which is never changed.
* token (String): this is the temporary link token as defined in the [Resources], send by the end user application during the link request.

If the auth token changes, the connector MUST send a registration request ([org.unifiedpush.android.distributor.REGISTER]) for all the registrations it has subscribed.
