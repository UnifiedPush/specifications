...
- [Messaging Broadcast Receiver](#messaging-broadcast-receiver)
  - [org.unifiedpush.android.connector.LINKED](#orgunifiedpushandroidconnectorlinked)
...
* Temporary link Token: This is a randomly generated token, generated by the connector for a link request. The end user application use it to link to a distributor, to receive the auth_token. The distributor don't have to save this token. The connector can remove this token once the auth_token is received. This token must contain sufficient entropy so it cannot be guessed. To generate this token, UUIDv4 ([RFC9562]) is suggested. It is unique on distributor and connector side. This is a string of maximum 100 bytes.
* Auth Token: This is a randomly generated token, generated by the distributor during a link request (via broadcast or link activity). The end user application use it to request new registrations. Its aim is to ensure that registration requests originate from the legitimate application, to prevent an illegitimate application from usurping the end user application ID in order to spam the distributor with registration requests. This token must contain sufficient entropy so it cannot be guessed. To generate this token, UUIDv4 ([RFC9562]) is suggested. It is unique on distributor and connector side. This is a string of maximum 100 bytes.
* Connection Token: This is a randomly generated token to identify the registration from the connector and the distributor. This token is generated by the connector, and send for the first time during the registration intent ([org.unifiedpush.android.distributor.REGISTER]). This token must contain sufficient entropy so it cannot be guessed. To generate this token, UUIDv4 ([RFC9562]) is suggested. It is unique on distributor and connector side. Except the linked request, every requests to the connector contains this token. If the connector doesn't know this token, the request is ignored. This is a string of maximum 100 bytes.
...
1. C enables Unifiedpush for the first time
    ...
    2. D: [Link Activity] opens and set result to RESULT_OK with the distributor package name (extra: application)
2. C request one or more registration, auth is always the same, token is different for each registration.
    1. C->D: [org.unifiedpush.android.distributor.REGISTER] (extra: auth_token, application, token)
...
6. C and D link each other in the background (can be used if the user select manually a distributor, or after a registration fail with UNAUTH)
...
7. D. checks a registration with C. that hasn't be used for weeks (~ping):
...
8. C. hasn't acknowledge any message nor ping (point 5.) since 30days, or the user logout of its distributor, D unregister the registration:
    1. C<-D: [org.unifiedpush.android.connector.UNREGISTERED] (extra: token)
9. C. tries to register but the registration fail
    1. C->D: [org.unifiedpush.android.distributor.REGISTER] (extra: auth_token, application, token)
    ...
    3. reason may be:
        ...
        2. "UNAUTH": C tries to register with an unknown auth_token, C must send a link request (point 6.) and try again to register with the new auth_token (point 2.)
...
The connector sends this action to link to a distributor. The intent MUST contain 1 extra:
* application (String): the end user application package name.
* link_token (String): this is the temporary link token as defined in the [Resources].

The distributor SHOULD check the application contained in the extra is installed on the device and exposes [org.unifiedpush.android.connector.LINKED] action in its manifest. If the application is not installed or do not expose the action, a warning about unusual activity may be shown to the user, and the registration MUST be ignored.

When it handles this action, the distributor MUST send a broadcast intent to [org.unifiedpush.android.connector.LINKED].

The distributor generate a single auth_token, as defined in the [Resources], for an application. If it receives a new LINK request for the same application, the distributor MUST send the same token.

The end user application SHOULD only send this action the first time it tries to use the distributor, or if it receives a [org.unifiedpush.android.connector.REGISTRATION_FAILED] with the reason "UNAUTH".

### org.unifiedpush.android.distributor.REGISTER

The connector sends this action to register to push messages. The intent MUST contain 3 extras:
...
If the intent's data and the callingPackage exist, the result is set to RESULT_OK (-1) with an intent containing the extra:
* application (String) the distributor package Name.
...
Following a link request, the distributor send this action to the connector.

This intent MUST contain 2 extras:
* auth_token (String): this is the auth token as defined in the [Resources]. This is a uniq token generated by the distributor, which is never changed.
* link_token (String): this is the temporary link token as defined in the [Resources], send by the end user application during the link request.

If the auth_token changes, the connector MUST send a registration request ([org.unifiedpush.android.distributor.REGISTER]) for all the registrations it has subscribed.

If the end user application doesn't know the temporary link token, it MUST ignore the intent.
...
* "UNAUTH": If the auth token does not match the one generated by the distributor, or if the registration request was sent before a link request.
